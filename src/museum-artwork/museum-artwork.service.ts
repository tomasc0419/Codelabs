...
export class MuseumArtworkService {
  ...
  async findArtworkByMuseumIdArtworkId(
    ...
    const museumArtwork = museum.artworks.find(
      (e) => e.id === artwork.id,
    );
  ...
  async associateArtworksMuseum(
    ...
    for (const artworkItem of artworks) {
      const artwork = await this.artworkRepository.findOne({
        where: { id: artworkItem.id },
      });
  ...
  async deleteArtworkMuseum(
    ...
    museum.artworks = museum.artworks.filter(
      (e) => e.id !== artworkId,
    );
